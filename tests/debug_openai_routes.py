...
async def run_test_scenario(client, scenario: str, endpoint: str, stream: bool) -> bool:
    ...
    if scenario == "basic":
        ...
        prompt = "Open https://openai.com/index/introducing-gpt-oss/ and tell me the announcement date formatted as YYYY-MM-DD."
        messages = [{"role": "user", "content": prompt}]
    ...
    try:
        ...
        if scenario == "basic":
            ...
            success = "2025-08-05" in content_norm.lower()

        elif scenario == "native_browser_search":
            # Search result check
            # Relaxed check: look for citation markers OR mentions of results
            success = (
                "python" in content_norm
                and ("result" in content_norm or "found" in content_norm)
            ) or "【" in content_norm

        elif scenario == "function":
            # Expect tool call 'get_stock_price'
            success = (
                "get_stock_price" in str(tool_calls)
                or "get_stock_price" in content_norm
            )
            ...
            # Expect a color
            success = any(
                c in content_norm for c in ["red", "green", "blue"]
            ) or "grammar_tool" in str(tool_calls)
